...
Layouts are React components in `layouts/**/*.tsx`. Registry-installed
layouts live in `layouts/campfire/`. A slide selects one by name:

```yaml
layout: problem-solution
```

resolves `layouts/problem-solution.tsx`, falling back to
`layouts/campfire/problem-solution.tsx`. When both exist, the user file wins
(with a `layout-collision` warning).
...
- Layouts stay pure: no presentation state, no runtime hooks. Current slide,
  ...
  layouts portable.
- Prefer narrative names (`title`, `section`, `problem-solution`, `roadmap`,
  `quote`, `closing`) over generic grids.
